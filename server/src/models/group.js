...
class GroupModel {
  ...
  // 删除中转站的所有分组和模型
  static deleteAllGroupsAndModels(proxyId) {
        ...
        db.run('BEGIN TRANSACTION', async (err) => {
          ...
          try {
            ...
            for (const group of groups) {
              // 删除分组的模型
              await new Promise((resolve, reject) => {
                db.run('DELETE FROM models WHERE groupId = ?', [group.id], function(err) {
                  if (err) {
                    reject(err);
                  } else {
                    deletedModels.push({
              ...
              // 删除分组
              await new Promise((resolve, reject) => {
                db.run('DELETE FROM groups WHERE id = ?', [group.id], function(err) {
                  if (err) {
                    reject(err);
                  } else {
            ...
            db.run('COMMIT', (err) => {
              ...
              resolve({
                ...
                deletedModels: deletedModels.reduce((total, item) => total + item.count, 0),
                message: `已删除 ${deletedGroups.length} 个分组和相关模型`
              });
            });
          } catch (error) {
            // 回滚事务
