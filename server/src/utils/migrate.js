...
const migrateDatabase = () => {
  return new Promise((resolve, reject) => {
    ...
    db.all("PRAGMA table_info(groups)", [], (err, columns) => {
      ...
      if (!hasRemark) {
        console.log('需要添加remark列到groups表...');
        
        // 添加remark列
        db.run("ALTER TABLE groups ADD COLUMN remark TEXT", (err) => {
          if (err) {
            console.error('添加remark列失败:', err);
            return reject(err);
          }
...
// 检查traces表是否需要迁移
const checkTracesTable = (resolve, reject) => {
